...
Usage examples:
    python run_experiments.py                # run all four experiments
...
def parse_cli(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "experiments", nargs="*",
        help="Experiments to run (A B C D). Empty = all.",
    )
...
def run_experiment(key: str, args: argparse.Namespace, no_build: bool) -> int:
...
def main(argv: list[str] | None = None) -> int:
    ...
    for i, key in enumerate(selected):
        ret = run_experiment(key, args, no_build=no_build)
        if ret != 0:
            rc = ret
