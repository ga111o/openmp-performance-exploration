...
본 Appendix에서는 실험에 사용한 주요 코드들의 주요 클래스 및 함수를 수록한다. 실험 데이터 측정, OpenMP 병렬 커널 구현 및 그리드 서치 자동화 스크립트를 포함한 전체 소스 코드 및 결과, figure는 다음 GitHub repository에서 확인 가능하다. \url{https://github.com/ga111o/openmp-performance-exploration}

\subsection{\texttt{main.c}}
\label{app:code-main}
핵심 계산 커널, naive prime 판별
\lstinputlisting[basicstyle=\ttfamily\scriptsize,firstline=59,lastline=68]{../scripts/main.c}

실험 파라미터를 커널 설정으로 변환
\lstinputlisting[basicstyle=\ttfamily\scriptsize,firstline=127,lastline=160]{../scripts/main.c}

작업 분배 방식별 병렬 실행 측정 분기
\lstinputlisting[basicstyle=\ttfamily\scriptsize,firstline=528,lastline=563]{../scripts/main.c}

\subsection{\texttt{run\_experiments.py}}
실행 옵션/SMT 모드 파싱
\lstinputlisting[basicstyle=\ttfamily\scriptsize,firstline=126,lastline=203]{../scripts/run_experiments.py}

실험 오케스트레이션 메인 루프
\lstinputlisting[basicstyle=\ttfamily\scriptsize,firstline=206,lastline=251]{../scripts/run_experiments.py}

\subsection{\texttt{grid\_search.py}}
유효 조합 정규화 및 중복 제거
\lstinputlisting[basicstyle=\ttfamily\scriptsize,firstline=139,lastline=183]{../scripts/grid_search.py}

OpenMP 런타임 환경 변수 구성 및 CPU set 결정
\lstinputlisting[basicstyle=\ttfamily\scriptsize,firstline=197,lastline=213]{../scripts/grid_search.py}
\lstinputlisting[basicstyle=\ttfamily\scriptsize,firstline=241,lastline=264]{../scripts/grid_search.py}

실행/재개/CSV 저장 파이프라인
\lstinputlisting[basicstyle=\ttfamily\scriptsize,firstline=341,lastline=420]{../scripts/grid_search.py}
